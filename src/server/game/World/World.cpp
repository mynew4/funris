...
void World::LoadConfigSettings(bool reload)
{
    ...
    rate_values[RATE_PVP_RANK_EXTRA_HONOR] = sConfig->GetFloatDefault("Rate.Rank.ExtraHonor", 1.0f);
	...
	rate_values[RATE_HACK_ARP] = sConfig->GetFloatDefault("HackARP", 1.0f);
    ...
    m_bool_configs[CONFIG_DUEL_RESET_COOLDOWN_ON_FINISH] = sConfig->GetBoolDefault("DuelResetCooldownOnFinish", false);
    //Multiboxing checking system
    m_bool_configs[CONFIG_MULTIBOXING_KICK_ENABLE] = sConfig->GetBoolDefault("Multiboxing.Kick.Enable", false);
     // Disable Chat
    m_int_configs[CONFIG_INT_CHAT_DISABLE_TIME]  = sConfig->GetIntDefault("Chat.DisableWhenPlayerCreate", 60);
    ...
    m_bool_configs[CONFIG_ANTIPERELIV]                        = sConfig->GetBoolDefault("AntiPereliv", false);
    m_bool_configs[CONFIG_ANTIDODGE]                          = sConfig->GetBoolDefault("AntiDodge", false);
    ...
    m_int_configs[CONFIG_ARENA_SPECTATOR_PRICE_HONOR] = sConfig->GetIntDefault("ArenaSpectator.HonorPrice", 5000);
    ...
    m_bool_configs[CONFIG_BG_GIVEMARKS]                              = sConfig->GetBoolDefault("Battleground.GiveMarks", false);
    ...
    m_bool_configs[CONFIG_ARENA_2v2_BRACKET_ENABLE]                  = sConfig->GetBoolDefault("Arena.2v2.Bracket.Enable", true);
    m_bool_configs[CONFIG_ARENA_3v3_BRACKET_ENABLE]                  = sConfig->GetBoolDefault("Arena.3v3.Bracket.Enable", true);
    m_bool_configs[CONFIG_ARENA_5v5_BRACKET_ENABLE]                  = sConfig->GetBoolDefault("Arena.5v5.Bracket.Enable", true);
    m_bool_configs[CONFIG_ARENA_MARK_OF_WIN_ENABLE]                  = sConfig->GetBoolDefault("Arena.MarkOfWin.Enable", false);
    ...
    m_bool_configs[CONFIG_CONSUMING_REAGENTS] = sConfig->GetBoolDefault("Prof.ConsumingReagents", true);
   ...
   // modifier online characters
    m_bool_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_ENABLE]   = sConfig->GetBoolDefault("ModifierOnlineCharacters.Enable", true);
    m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_MIN_TIMER] = (sConfig->GetIntDefault("ModifierOnlineCharacters.MinTimer", 60) * MINUTE * IN_MILLISECONDS);
    m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_MAX_TIMER] = (sConfig->GetIntDefault("ModifierOnlineCharacters.MaxTimer", 180) * MINUTE * IN_MILLISECONDS);
    m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_MIN_MONEY] = sConfig->GetIntDefault("ModifierOnlineCharacters.MinMoney", 100);
    m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_MAX_MONEY] = sConfig->GetIntDefault("ModifierOnlineCharacters.MaxMoney", 200);
	...
	// Custom
	m_bool_configs[CONFIG_FREE_TRANSMOGRIFICATION] = sConfig->GetBoolDefault("Free.Transmogrification", false);
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        if (m_updateTimeSum > m_int_configs[CONFIG_INTERVAL_LOG_UPDATE])
        {
            ...
            LoginDatabase.PExecute("UPDATE realmlist set population%u where id=%u", GetActiveSessionCount(), realmID);
...
if (m_timers[WUPDATE_MODIFIER_ONLINE_CHARACTERS].Passed())
    {
        ...
        if (getBoolConfig(CONFIG_MODIFIER_ONLINE_CHARACTERS_ENABLE))
        {
            ...
            for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
            {
                ...
                session->GetPlayer()->ModifyArenaPoints(sendMoneyCount);
