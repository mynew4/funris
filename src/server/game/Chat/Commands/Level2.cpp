...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    if (session && session->GetSecurity() < SEC_MODERATOR)
        if (notspeaktime > 60)
            notspeaktime = 60;
...
bool ChatHandler::HandleCharacterChangeFactionCommand(const char * args)
{
    ...
    if(target)
    {
		...
		CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = 32182 AND guid = %u", target->GetGUIDLow()); //unlearn Heroism
		CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = 2825 AND guid = %u", target->GetGUIDLow()); //unlearn Bloodlust
    }
    else
    {
        std::string oldNameLink = playerLink(target_name);
		...
		CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = 32182 AND guid = %u", GUID_LOPART(target_guid)); //unlearn Heroism
		CharacterDatabase.PExecute("DELETE FROM character_spell WHERE spell = 2825 AND guid = %u", GUID_LOPART(target_guid)); //unlearn Bloodlust
    }
