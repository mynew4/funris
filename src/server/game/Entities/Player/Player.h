...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        uint32 GetSpecifiedVendorEntry (uint32 vendorslot, uint32 item);
        ...
        bool isRussianLocale() const { return GetSession()->GetSessionDbcLocale() == LOCALE_ruRU;}
        ...
        void GroupKillHappens(uint32 entry, WorldObject const* pEventObject, uint64 creatureGUID = 0);
        ...
        { 
            if (isRaid)
            {
                if (sWorld->getBoolConfig(CONFIG_INSTANCE_ONLY_HEROIC))
            ...
            {
                if (sWorld->getBoolConfig(CONFIG_INSTANCE_ONLY_HEROIC))
                    return DUNGEON_DIFFICULTY_HEROIC;
                else
                    return m_dungeonDifficulty;
            }
        }
        Difficulty GetDungeonDifficulty() const { return sWorld->getBoolConfig(CONFIG_INSTANCE_ONLY_HEROIC) ? DUNGEON_DIFFICULTY_HEROIC : m_dungeonDifficulty; }
        Difficulty GetRaidDifficulty() const { return sWorld->getBoolConfig(CONFIG_INSTANCE_ONLY_HEROIC) ? RAID_DIFFICULTY_10MAN_HEROIC : m_raidDifficulty; }
        Difficulty GetStoredRaidDifficulty() const { return sWorld->getBoolConfig(CONFIG_INSTANCE_ONLY_HEROIC) ? RAID_DIFFICULTY_10MAN_HEROIC : m_raidMapDifficulty; } // only for use in difficulty packet after exiting to raid map
