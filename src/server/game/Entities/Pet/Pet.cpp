...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current)
{
    ...
    switch (getPetType())
    {
        case SUMMON_PET:
            ...
            // Glyph of Seduction
            if(owner->HasAura(56250))
                CastSpell(this, 56250, true);
            break;
        case HUNTER_PET:
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (cFamily && cFamily->minScale > 0.0f && petType == HUNTER_PET)
    {
        ...
        // no more cute pets
        if(this->ToCreature()->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_EXOTIC)
            scale*=1.5f;
        SetFloatValue(OBJECT_FIELD_SCALE_X, scale);
        SetFloatValue(UNIT_FIELD_COMBATREACH, scale*2);
    }
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 1964: //force of nature
                {
                    ...
                    if(!HasAura(50419))
                        if(AuraEffect * Auraeff = m_owner->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_DRUID, 53, 2))
                        {
                            Aura * aur = AddAura(50419, this);
                            aur->GetEffect(0)->SetAmount(Auraeff->GetAmount());
                        }
                    break;
                }
                ...
                case 29264: // Feral Spirit
                {
                    ...
                    SetBonusDamage(int32(m_owner->GetTotalAttackPowerValue(BASE_ATTACK, NULL) * dmg_multiplier));
                ...
                case 28017: // Bloodworms
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - 30 - (petlevel / 4)) + m_owner->GetTotalAttackPowerValue(BASE_ATTACK, 0) * 0.006f);
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel - 30 + (petlevel / 4)) + m_owner->GetTotalAttackPowerValue(BASE_ATTACK, 0) * 0.006f);
                }
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    SetBonusDamage(int32(m_owner->GetTotalAttackPowerValue(BASE_ATTACK, NULL) * 0.7f));
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
