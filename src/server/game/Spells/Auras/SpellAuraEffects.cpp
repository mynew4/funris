                        ...
                        if (item_rand_suffix)
                        {
                            for (int k = 0; k < MAX_ITEM_ENCHANTMENT_EFFECTS; k++)
                            {
                                ...
                                if (pEnchant)
                                {
                                    ...
                                    {
                                        amount = uint32((item_rand_suffix->prefix[k]*castItem->GetItemSuffixFactor()) *0.0001f);
                                        break;
                                    }
                                }
    ...
    switch(GetAuraType())
    {
        ...
        case SPELL_AURA_SCHOOL_ABSORB:
            ...
            switch(GetSpellProto()->SpellFamilyName)
            {
                case SPELLFAMILY_GENERIC:
                    // Shield Generator
                    if(GetSpellProto()->Id == 64677)
                    {
                        DoneActualBenefit = GetBase()->GetUnitOwner()->GetMaxHealth() * GetSpellProto()->EffectBasePoints[0];
                    }
                    break;
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
            ...
            else if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARRIOR && GetSpellProto()->SpellFamilyFlags[0] & 0x20)
            {
                ...
                float mwb = ((mwb_min + mwb_max) * 0.5f + ap * mws / 14000) * 0.2f;
        ...
        case SPELL_AURA_MOD_INCREASE_ENERGY:
            ...
            if (GetId() == 64904)
            {
                AuraEffect * aur = NULL; 
                if (caster)
                    aur = target->GetAuraEffect(64904, 1, caster->GetGUID());

                if (aur)
                    amount = aur->GetAmount();
                else
                    amount = target->GetMaxPower(POWER_MANA) * amount * 0.01f;
            }
            break;
...
void AuraEffect::CalculatePeriodic(Unit * caster, bool create)
{
    ...
    if (m_amplitude)
    {
        ...
        if (caster)
        {
            ...
            if (IsChanneledSpell(m_spellProto) && m_spellProto->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECTS_CHANNEL_TIME)
    ...
    switch(GetAuraType())
    {
            ...
            if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                ...
                if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARLOCK && (GetSpellProto()->SpellFamilyFlags[0] & 0x400) && GetSpellProto()->SpellIconID == 544)
                {
                    ...
                    // 9..12 ticks, 3/2 from normal tick damage
                        ...
                        damage += (damage+1) * 0.5f;           // +1 prevent 0.5 damage possible lost at 1..4 ticks
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            else
            {
                ...
                {
                    damage += int32(float(damage * GetTotalTicks()) * ((6-float(2*(GetTickNumber()-1)))*0.01f));
                }
        ...
        case SPELL_AURA_OBS_MOD_POWER:
        {
            ...
            uint32 amount = m_amount * target->GetMaxPower(power) *0.01f;
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellProto()->Id)
            {
                ...
                case 22842:
                {
                    ...
                    int32 regen = target->GetMaxHealth() * (mod * points * 0.1f) *0.001f;
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {   // hacky, but the only way as spell family is not SPELLFAMILY_DRUID
                ...
                for (Unit::AuraEffectList::const_iterator i = mModTotalStatPct.begin(); i != mModTotalStatPct.end(); ++i)
                {
                    ...
                    {
                        int32 HotWMod = HotWSpellId == 24900 ? (*i)->GetAmount()/2 : (*i)->GetAmount(); //fix x2 attack power bonus by Heart of the Wild
...
void AuraEffect::HandleAuraDummy(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    switch(m_spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_HUNTER:
            // Animal Handler
            if(m_spellProto->SpellIconID == 2234 && target->GetTypeId()==TYPEID_PLAYER)
                if(Pet *pet=target->ToPlayer()->GetPet())
                {
                    pet->RemoveAurasDueToSpell(68361);
                    if(apply)
                        pet->CastCustomSpell(pet, 68361, &m_amount, 0, 0, true);
                }
            break;
        case SPELLFAMILY_GENERIC:
        {
